...
-- Deck mode: navigation + entity editing (no DB import/generate)
function M.setup_deck()
	setup_navigation()

	-- Entity commands — YAML editing works without DB; import skipped in deck mode
	vim.api.nvim_create_user_command("PalimpsestEdit", function()
		require("palimpsest.entity").edit()
	end, { desc = "Edit current entity metadata in floating window" })

	vim.api.nvim_create_user_command("PalimpsestEditCuration", function()
		require("palimpsest.entity").edit_curation()
	end, { desc = "Edit curation file for current entity type" })
