...
local function import_yaml(filepath)
	...
	local cmd = string.format(
		"cd %s && plm metadata import %s && plm wiki generate --section %s && plm wiki generate --section indexes && plm export --no-commit",
...
local function import_after_rename(entity_type)
	...
	local cmd = string.format(
		"cd %s && %s && plm wiki generate --section manuscript && plm wiki generate --section indexes && plm export --no-commit",
		root, import_chain
	)
	vim.fn.jobstart(cmd, {
		on_exit = function(_, exit_code)
