...
yaml_formatter.py
-----------------
YAML formatting utilities for Palimpsest narrative analysis files.

Provides consistent formatting for YAML files following project standards:
- Block scalars (>-) for long content (>80 chars per line)
- Auto-wrapping at natural boundaries (periods, commas)
- Block style for arrays in md_frontmatter
- Empty array/list removal
- Consistent quote handling

Key Features:
    - YAMLFormatter class for formatting dictionaries to YAML strings
    - Preserves paragraph breaks in long text
    - Handles nested structures (scenes, threads, events, motifs)
    - Removes empty fields automatically

Usage:
    from dev.utils.yaml_formatter import YAMLFormatter

    formatter = YAMLFormatter()
    formatted_yaml = formatter.format_dict(data)

    # Or format a complete document with field-specific handling
    cleaned_data = formatter.format_document(data)
    output = formatter.format_dict(cleaned_data)
"""
# --- Annotations ---
...
class YAMLFormatter:
    """
    Formats YAML data according to Palimpsest project standards.

    Applies consistent formatting rules:
    - Use >- block scalars for long content (>80 chars or contains quotes)
    - Auto-wrap long lines at natural boundaries
    - Remove empty arrays and None values
    - Convert dates to strings

    Attributes:
        wrapper: TextWrapper instance for line wrapping
    """
    ...
    def should_use_block_scalar(self, text: str) -> bool:
        ...
        """
        # Use block scalar if long or has paragraph breaks or contains quotes
    ...
    def format_scenes(self, scenes: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        Returns:
            Formatted scenes with cleaned fields
        """
        if not scenes:
            return scenes
    ...
    def format_threads(self, threads: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        Returns:
            Formatted threads with date fields as strings
        """
        if not threads:
            return threads
    ...
    def format_events(self, events: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        Returns:
            Formatted events with only name and scenes
        """
        if not events:
            return events
    ...
    def format_document(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        Returns:
            Formatted document with cleaned fields
        """
