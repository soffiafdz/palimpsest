...
class TestWikiSetupCLI:
    """Tests for ``plm wiki setup``."""

    def test_setup_help(self, runner):
        """Help text works for setup command."""
        result = runner.invoke(cli, ["wiki", "setup", "--help"])
        assert result.exit_code == 0
        assert "Install Quartz" in result.output


class TestWikiServeCLI:
    """Tests for ``plm wiki serve``."""

    def test_serve_help(self, runner):
        """Help text works for serve command."""
        result = runner.invoke(cli, ["wiki", "serve", "--help"])
        assert result.exit_code == 0
        assert "Publish wiki and serve" in result.output

    def test_serve_fails_without_quartz(self, runner, monkeypatch):
        """Serve fails gracefully when Quartz is not installed."""
        from dev.core import paths

        monkeypatch.setattr(paths, "QUARTZ_DIR", paths.ROOT / "nonexistent")
        result = runner.invoke(cli, ["wiki", "serve"])
        assert result.exit_code != 0
        assert "plm wiki setup" in result.output
