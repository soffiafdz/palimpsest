...
class TestDateManagerGetForEntry:
    """Test DateManager.get_for_entry() method."""

    def test_get_for_entry_returns_linked_dates(self, date_manager, db_session):
        """Test get all mentioned dates for an entry."""
        entry = Entry(date=date(2024, 1, 1), file_path="/path/to/entry-2024-01-01.md")
        date1 = MentionedDate(date=date(2024, 6, 15))
        date2 = MentionedDate(date=date(2024, 7, 20))
        date3 = MentionedDate(date=date(2024, 8, 10))
        date1.entries.append(entry)
        date2.entries.append(entry)
        db_session.add_all([entry, date1, date2, date3])
        db_session.commit()

        result = date_manager.get_for_entry(entry)

        assert len(result) == 2
        dates = [d.date for d in result]
        assert date(2024, 6, 15) in dates
        assert date(2024, 7, 20) in dates

    def test_get_for_entry_returns_in_chronological_order(self, date_manager, db_session):
        """Test get_for_entry returns dates in chronological order."""
        entry = Entry(date=date(2024, 1, 1), file_path="/path/to/entry-2024-01-01.md")
        date1 = MentionedDate(date=date(2024, 8, 10))
        date2 = MentionedDate(date=date(2024, 6, 15))
        date3 = MentionedDate(date=date(2024, 7, 20))
        date1.entries.append(entry)
        date2.entries.append(entry)
        date3.entries.append(entry)
        db_session.add_all([entry, date1, date2, date3])
        db_session.commit()
        ...
        dates = [d.date for d in result]
        assert dates == [date(2024, 6, 15), date(2024, 7, 20), date(2024, 8, 10)]


class TestDateManagerGetForLocation:
    """Test DateManager.get_for_location() method."""

    def test_get_for_location_returns_linked_dates(self, date_manager, db_session):
        """Test get all mentioned dates for a location."""
        city = City(city="France")
        db_session.add(city)
        db_session.flush()
        ...
        date2 = MentionedDate(date=date(2024, 7, 20))
        date3 = MentionedDate(date=date(2024, 8, 10))
        date1.locations.append(location)
        date2.locations.append(location)
        db_session.add_all([location, date1, date2, date3])
        db_session.commit()

        result = date_manager.get_for_location(location)
    ...
    def test_get_for_location_returns_in_chronological_order(self, date_manager, db_session):
        """Test get_for_location returns dates in chronological order."""
        city = City(city="France")
        db_session.add(city)
        db_session.flush()
        ...
        date1.locations.append(location)
        date2.locations.append(location)
        date3.locations.append(location)
        db_session.add_all([location, date1, date2, date3])
        db_session.commit()
...
class TestDateManagerGetForPerson:
    """Test DateManager.get_for_person() method."""

    def test_get_for_person_returns_linked_dates(self, date_manager, db_session):
        """Test get all mentioned dates for a person."""
        person = Person(name="Alice")
        date1 = MentionedDate(date=date(2024, 6, 15))
        date2 = MentionedDate(date=date(2024, 7, 20))
        date3 = MentionedDate(date=date(2024, 8, 10))
        date1.people.append(person)
        date2.people.append(person)
        db_session.add_all([person, date1, date2, date3])
        db_session.commit()

        result = date_manager.get_for_person(person)
    ...
    def test_get_for_person_returns_in_chronological_order(self, date_manager, db_session):
        """Test get_for_person returns dates in chronological order."""
        person = Person(name="Alice")
        date1 = MentionedDate(date=date(2024, 8, 10))
        date2 = MentionedDate(date=date(2024, 6, 15))
        date3 = MentionedDate(date=date(2024, 7, 20))
