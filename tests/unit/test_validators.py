...
class TestNormalizeReferenceMode:
    """Test normalize_reference_mode method."""

    def test_valid_reference_mode(self):
        """Test valid reference mode is normalized."""
        from dev.database.models import ReferenceMode
        result = DataValidator.normalize_reference_mode("direct")
        assert result == ReferenceMode.DIRECT

    def test_none_returns_none(self):
        """Test None returns None."""
        assert DataValidator.normalize_reference_mode(None) is None


class TestNormalizeReferenceType:
    """Test normalize_reference_type method."""

    def test_valid_reference_type(self):
        """Test valid reference type is normalized."""
        from dev.database.models import ReferenceType
        result = DataValidator.normalize_reference_type("book")
        assert result == ReferenceType.BOOK

    def test_none_returns_none(self):
        """Test None returns None."""
        assert DataValidator.normalize_reference_type(None) is None


class TestNormalizeRelationType:
    """Test normalize_relation_type method."""

    def test_valid_relation_type(self):
        """Test valid relation type is normalized."""
        from dev.database.models import RelationType
        result = DataValidator.normalize_relation_type("family")
        assert result == RelationType.FAMILY

    def test_none_returns_none(self):
        """Test None returns None."""
        assert DataValidator.normalize_relation_type(None) is None
